...
声纳探测基于HC-SR04超声波模块，通过发射超声波并测量反射回来的时间计算距离。工作原理是：
- 发送10μs以上的脉冲触发测量
- 模块自动发送8个40kHz的方波
- 接收到反射波后ECHO输出高电平
- 高电平持续时间等于超声波往返时间
...
1. `Hcsr04_Init()` - 初始化超声波模块和相关定时器
2. `Hcsr04_GetDistance()` - 获取滤波后的距离值
3. `Hcsr04_FilteredMeasurement()` - 执行多次测量并进行滤波处理
4. `Hcsr04_MeasureWithTimeout()` - 带超时机制的单次测量
...
{  
    // 配置GPIO引脚 - PB10为输出(TRIG), PB11为输入(ECHO)
    // 配置定时器4用于时间测量
    // 配置中断优先级
}
```

### 2. 核心测量函数 - Hcsr04_MeasureWithTimeout()
...
### 3. 滤波测量函数 - Hcsr04_FilteredMeasurement()
...
1. **超时处理**: 避免ECHO信号未返回导致程序卡死
2. **多次测量**: 每次取5次测量结果的平均值
3. **异常值过滤**: 排序后去除最高和最低值
4. **测量间延时**: 确保每次测量有足够间隔
5. **动态避障**: 根据连续障碍物检测结果智能变向

## 注意事项

1. 声纳容易受环境影响（光滑表面、小物体、布料等）
2. 使用前确保TRIG和ECHO引脚连接正确
3. 测量周期建议≥60ms，避免回波干扰
4. 实际使用中最佳测距范围为2cm-200cm
